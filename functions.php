...
if ( ! function_exists( 'knight_setup' ) ) :
	...
	function knight_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'knight' ),
			)
		);
...
function knight_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'knight_content_width', 640 );
}
...
function knight_scripts() {
	wp_enqueue_style( 'knight-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'knight-style', 'rtl', 'replace' );

	wp_enqueue_script( 'knight-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
